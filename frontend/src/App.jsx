...
const API_BASE_URL =
  ...
  "https://speech-to-text-api-u2tn.onrender.com";
...
export default function App() {
    ...
    if (!isRecording) {
      try {
        ...
        mediaRecorder.current.onstop = async () => {
          ...
          const file = new File([blob], `speech-${timestamp}.${fileExt}`, { type: blob.type });

          stream.getTracks().forEach((track) => track.stop());

          console.log("Recording finished:", file.name, file.type, blob.size);
  ...
  const downloadTxt = () => {
    ...
    element.download = `transcript-${new Date().toISOString().replace(/:/g, "-")}.txt`;
    document.body.appendChild(element);
    element.click();
  };

  const downloadHistoryItem = (text) => {
    const element = document.createElement("a");
        ...
        <div className="neon-card pt-16 pb-12 px-10 rounded-[2.5rem] w-full max-w-[380px] text-center border border-white/5 shadow-2xl bg-[#0d1117]">
          <div className="flex justify-center items-end gap-2 h-14 mb-8 mt-4">
            ...
            {[
              ...
              <div
                ...
                style={{
                  height: "10px",
                  animation: "true-wave 1.5s ease-in-out infinite",
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <input
              ...
              value={email}
              placeholder="Email"
              className="w-full bg-[#05070a] border border-white/10 p-4 rounded-xl text-xs text-white outline-none focus:border-cyan-500"
            ...
            <button
              type="submit"
              className="w-full bg-cyan-600 text-white font-black py-4 rounded-xl uppercase tracking-widest text-[10px]"
            >
          ...
          <button
            ...
            className="mt-8 text-[9px] text-slate-500 hover:text-white uppercase font-bold tracking-widest transition-colors"
          >
            {isSignup
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#05070a] text-white">
      ...
      <nav className="h-16 px-10 border-b border-white/5 flex items-center justify-between sticky top-0 bg-[#05070a] z-50">
        <span className="text-cyan-400 text-base font-bold tracking-[0.25em] opacity-90 italic">
          Real-time speech intelligence at your fingertips.
        </span>
        <div className="flex gap-4">
          <button
            onClick={() => setShowHistory(!showHistory)}
            className="px-5 py-2 rounded-lg bg-cyan-600 text-white text-[9px] font-black uppercase"
          >
            History
          </button>
          <button
            ...
            className="px-5 py-2 rounded-lg border border-red-900/30 text-red-500 text-[9px] font-bold uppercase hover:bg-red-950/20"
          >
            Logout
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center">
        <div className="flex items-center justify-center min-h-[calc(100vh-64px)] w-full">
          <div className="w-full max-w-md bg-[#0d1117] border border-white/5 rounded-[2rem] p-8 flex flex-col items-center shadow-2xl">
            <h2 className="text-[10px] font-bold tracking-[0.4em] text-slate-500 uppercase mb-6">
              ...
              {isRecording ? (
                [...Array(12)].map((_, i) => (
                  <div
                    ...
                    style={{
                      height: "10px",
                      animation: "natural-ripple 1.4s ease-in-out infinite",
            ...
            <div className="w-full bg-black/40 border border-white/5 rounded-[1.5rem] p-6 min-h-[120px] flex flex-col items-center justify-center mb-8 text-center">
              {loading ? (
                <span className="animate-pulse text-cyan-500 text-[10px] font-black uppercase tracking-widest">
                  Processing...
                </span>
              ) : (
              ...
              {transcript && !loading && (
                <button
                  onClick={downloadTxt}
                  className="mt-4 text-[10px] text-cyan-400 font-bold border border-cyan-400/30 px-6 py-2 rounded-full hover:bg-cyan-400/10 transition-all italic"
                >
                  📥 DOWNLOAD TRANSCRIPT
                </button>
              )}
            ...
            {/* Buttons */}
              <button
                ...
                disabled={loading}
                className={`w-full py-4 rounded-xl font-black text-[11px] tracking-[0.2em] transition-all shadow-lg ${
                  isRecording ? "bg-red-600 animate-pulse text-white" : "bg-cyan-600 text-white hover:bg-cyan-500"
        ...
        {showHistory && (
          <div className="w-full max-w-6xl px-10 pb-20 animate-in fade-in slide-in-from-top-4 duration-700">
            <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4">
              ...
              <button
                onClick={clearAll}
                className="text-[9px] text-red-500 font-bold uppercase hover:text-red-400 tracking-widest"
              >
                Clear All
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {history.length > 0 ? (
                history.map((item) => (
                  <div
                    key={item.id}
                    className="p-6 rounded-2xl bg-[#0d1117] border border-white/5 flex flex-col justify-between hover:border-cyan-500/40 transition-all shadow-xl min-h-[150px]"
                  >
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-white/5">
                      ...
                      <div className="flex gap-3">
                        <button
                          onClick={() => downloadHistoryItem(item.text)}
                          className="text-[8px] text-cyan-500/70 hover:text-cyan-400 font-bold uppercase transition-all"
                        >
                          Download
                        </button>
                        <button
                          onClick={() => deleteItem(item.id)}
                          className="text-[8px] text-red-500/50 hover:text-red-500 font-bold uppercase transition-all"
                        >
                          Delete
                        </button>
                      </div>
                ...
                <div className="col-span-full text-center py-10 opacity-20 text-xs uppercase tracking-widest italic">
